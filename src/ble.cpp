...
namespace net
{
    BLE::BLE(std::string deviceName, const BleConfig::Preset preset) :
        Transport(TAG),
        mConfig(preset),
    ...
    esp_err_t BLE::initialize()
    {
        ...
        if (mDeviceName.empty() || !mDataCallback)
        {
        ...
        ret = esp_ble_gap_set_device_name(mDeviceName.c_str());
        if (ret != ESP_OK)
        {
        ...
        ESP_LOGI(TAG, "BLE 5.0 initialized successfully. Device name: %s", mDeviceName.c_str());
        return ESP_OK;
    }

    esp_err_t BLE::quickStart(std::unique_ptr<PacketCallback> callback)
    {
    ...
    esp_err_t BLE::startAdvertising()
    {
        ...
        if (!isInitialized() || mServiceHandle == 0)
        {
    ...
    esp_err_t BLE::sendImpl(Packet& packet)
    {
        // Обработка broadcast
        if (packet.id == 0)
        {
            ...
            for (const auto& [connId, address] : mActiveConnections)
            {
                if (const esp_err_t ret = sendToDevice(connId, packet.buffer, packet.size); ret != ESP_OK)
                {
                    finalRet = ret;
                }
            }
        ...
        return sendToDevice(packet.id, packet.buffer, packet.size);
    }
    ...
    size_t BLE::getMtuSize() const noexcept
    {
        std::lock_guard lock(mMutex);
        return mMtu;
    }
    ...
    esp_err_t BLE::startLegacyAdvertising()
    {
        ...
        if (const auto [len, uuid] = uuidFromString(mConfig.gatt.serviceUuid, mConfig.gatt.invertBytes); len ==
            ESP_UUID_LEN_128)
        {
    ...
    esp_err_t BLE::configureExtendedAdvertising()
    {
        ...
        if (const auto [uuidLen, uuid] = uuidFromString(mConfig.gatt.serviceUuid, mConfig.gatt.invertBytes); uuidLen ==
            ESP_UUID_LEN_16)
        {
            advData.insert(advData.end(), {
                               0x03, // Length
    ...
    void BLE::handleWriteEvent(const uint16_t connId, const esp_ble_gatts_cb_param_t* param)
    {
        if (param == nullptr)
        {
